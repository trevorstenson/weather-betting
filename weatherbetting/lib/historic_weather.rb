...
class HistoricWeather
    ...
    def self.get_data(lat, lon, date, start_time, end_time)
    ...
    def self.build_start_datetime(loc, date)
        return date.to_s + "T06:00:00" + @@locations[loc][2]
    end

    def self.build_middle_datetime(loc, date)
        return date.to_s + "T18:00:00" + @@locations[loc][2]
    end

    def self.build_end_datetime(loc, date)
